...
def delete_product(encrypted_id):
    try:
        ...
        if product_data:
            ...
            query_insert_into_respaldo = "INSERT INTO producto_respaldo (ID, Nombre, Descripcion, StockActual, fecha_eliminacion) VALUES (%s, %s, %s, %s, %s)"
            cursor.execute(query_insert_into_respaldo, (product_data[0], product_data[1], product_data[2], product_data[3], date.today()))
...
def create_user_window():
    def validate_rut_input(char, current_value):
        # Esta función se utilizará para validar la entrada del RUT en tiempo real
        return (char.isdigit() or char == "") and len(current_value) <= 8
    
    def validate_password(password):
        # Validar la contraseña
        # Aquí puedes agregar tus propios criterios para una contraseña segura
    ...
    def create_user(rut, name, password, user_type):
        # Aquí puedes realizar la creación del usuario con los datos proporcionados
        if validate_password(password):
            messagebox.showinfo("Usuario Creado", "¡Usuario creado con éxito!")
        else:
            messagebox.showwarning("Contraseña débil", "La contraseña no cumple con los requisitos mínimos.")

    create_user_window = tk.Toplevel()
    create_user_window.title("Crear Usuario")
    create_user_window.geometry("500x300")
    validate_rut = create_user_window.register(validate_rut_input) 

    rut_label = tk.Label(create_user_window, text="RUT (1-8 dígitos):")
    rut_label.pack()

    rut_entry = tk.Entry(create_user_window, validate="key", validatecommand=(validate_rut, "%S", "%P"))
    ...
    password_label = tk.Label(create_user_window, text="Contraseña (mínimo 8 caracteres, al menos una mayúscula y un número):")
    password_label.pack()
...
def menu_window():
    ...
    user_label = tk.Label(menu_window, text=f"Bienvenido, {current_user_permissions} {current_user_name}", anchor="e")
    user_label.pack(pady=5, padx=5)


    # Crear botones para las opciones del menú
    options = ["Añadir Productos", "Revisar Inventario(ADMIN)","Revisar Inventario(Lector)", "Crear Usuario", "Cerrar Sesión"]
...
def main_window():
    ...
    validate_rut = window.register(validate_rut_input)  # Registrar la función de validación
